...
def generate_launch_description():
    ...
    declare_lidar_cmd = DeclareLaunchArgument(
        "lidar",
        default_value="livox",
        description="LiDAR driver: 'livox' (mid360 + point_lio) or 'odin' (Odin1 built-in SLAM)",
    )
    ...
    declare_params_file_cmd = DeclareLaunchArgument(
        ...
        default_value=PythonExpression([
            "'", os.path.join(bringup_dir, "config", "reality", "odin1_nav2_params.yaml"),
            "' if '", lidar, "' == 'odin' else '",
            os.path.join(bringup_dir, "config", "reality", "nav2_params.yaml"), "'",
        ]),
        description="Full path to the ROS2 parameters file to use for all launched nodes "
                    "(auto-selected by 'lidar')",
    )
    ...
    start_odin_driver_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(launch_dir, "odin_driver_launch.py")),
