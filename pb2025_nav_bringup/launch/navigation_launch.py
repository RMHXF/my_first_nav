...
def generate_launch_description():
    ...
    declare_lidar_cmd = DeclareLaunchArgument(
        "lidar", default_value="livox", description="LiDAR driver: 'livox' or 'odin'"
    )
    ...
    load_nodes = GroupAction(
        ...
        actions=[
            ...
            Node(
                ...
                parameters=[configured_params],
                arguments=["--ros-args", "--log-level", log_level],
                condition=LaunchConfigurationEquals("lidar", "odin"),
            ),
            Node(
    ...
    # Interface node differs per LiDAR driver: loam_interface for livox,
    # odin_interface for Odin1.
    load_loam_interface_composable = LoadComposableNodes(
        condition=IfCondition(
            PythonExpression(["'", lidar, "' == 'livox' and ", use_composition])
        ),
        target_container=container_name_full,
        composable_node_descriptions=[
            ComposableNode(
    ...
    load_odin_interface_composable = LoadComposableNodes(
        condition=IfCondition(
            PythonExpression(["'", lidar, "' == 'odin' and ", use_composition])
        ),
        target_container=container_name_full,
        composable_node_descriptions=[
            ComposableNode(
                package="odin_interface",
                plugin="odin_interface::OdinInterfaceNode",
                name="odin_interface",
                parameters=[configured_params],
            ),
        ],
    ...
    ld.add_action(load_loam_interface_composable)
    ld.add_action(load_odin_interface_composable)
